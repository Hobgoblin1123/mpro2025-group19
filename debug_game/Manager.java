...
class MoveManager {
    ...
    public MoveManager(int x, int y, int offset, boolean server, String host, int port) {
        ...
        if (server) { // server の場合は，自分は左．clientの場合は自分は，右
            myself = new Player(10, 10, offset, y / 2, 5, 0, 200, y, 5);
            opponent = new Player(10, 10, x - offset - myself.getRadius(), y / 2, 5, 320, 600, y, 5);
            System.out.println("Waiting for connection with port no: " + port);
            sv = new CommServer(port);
            sv.setTimeout(1); // non-wait で通信
            System.out.println("Connected !");
        } else {
            opponent = new Player(10, 10, offset, y / 2, 5, 0, 200, y, 5);
            myself = new Player(10, 10, x - offset - opponent.getRadius(), y / 2, 5, 320, 600, y, 5);
