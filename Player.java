...
class Player {
    ...
    public ArrayList<Bullet> tryShoot(int type) {
        ...
        if (type == 0) {
            ...
            newBullets
                    .add(new Bullet(this, this.getX(), this.getY(), 1.80f, 15 + this.getBiggerbullet(), 1, Shootdir,
                            null));
        } else if (type == 1) {
            // 曲線弾
            GameFrame.playSE("music/Thunder_Shot.wav", 0.5f);
            newBullets.add(
                    new CurveBullet(this, this.getX(), this.getY(), 0.50f, 10 + this.getBiggerbullet(), 1, Shootdir,
                            null));
        } else if (type == 2) {
            // 斜め2方向弾
            GameFrame.playSE("music/Gun_Shot.wav", 0.5f);
            newBullets.add(new UpDiagonalBullet(this, this.getX(), this.getY(), 1.10f, 15 + this.getBiggerbullet(), 1,
                    Shootdir, null));
            newBullets.add(new DownDiagonalBullet(this, this.getX(), this.getY(), 1.10f, 15 + this.getBiggerbullet(), 1,
                    Shootdir, null));
        }
        return newBullets;
    }
